...
class Social_analytics extends Memcached_DataObject
{
     ...
     * TODO: Document 
     */
    static function init($user_id, $sdate, $edate)
    {
        ...
        foreach($mentions[$user_id] as $mention) {
            ...
            if($date_created >= $sa->sdate && $date_created <= $sa->edate) {
                $notice = Notice::getKV('id', $mention->notice_id);
                $profile = Profile::getKV('id', $notice->profile_id);

                if(!isset($sa->graphs['people_who_mentioned_you'][$profile->nickname])) {
        ...
        foreach($arr_following[$user_id] as $following) {
            ...
            if($date_created >= $sa->sdate && $date_created <= $sa->edate) {
                $profile = Profile::getKV('id', $following->subscribed);
                $sa->graphs['trends'][$date_created->format('Y-m-d')]['following'][] = $profile;

                if(isset($profile->lat) && isset($profile->lon)) {
                    $sa->map['following'][$profile->nickname] = '{ lon: ' . $profile->lon . ', lat: '  . $profile->lat . ', profileUrl: "' . $profile->profileurl . '"}';
                }

                $hst = parse_url($profile->profileurl, PHP_URL_HOST);
                
                if(!isset($sa->graphs['hosts_you_started_to_follow'][$hst])) {
        ...
        foreach($followers[$user_id] as $follower) {
            ...
            if($date_created >= $sa->sdate && $date_created <= $sa->edate) {
                $profile = Profile::getKV('id', $follower->subscriber);
                $sa->graphs['trends'][$date_created->format('Y-m-d')]['followers'][] = $profile;

                if(isset($profile->lat) && isset($profile->lon)) {
                    $sa->map['followers'][$profile->nickname] = '{ lon: ' . $profile->lon . ', lat: '  . $profile->lat . ', profileUrl: "' . $profile->profileurl . '"}';
                }

                $hst = parse_url($profile->profileurl, PHP_URL_HOST);

                if(!isset($sa->graphs['hosts_who_started_to_follow_you'][$hst])) {
